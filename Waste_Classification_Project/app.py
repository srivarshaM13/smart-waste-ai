...
# (These are approximate, just for project demo)
...
# ------------------------------
# INPUTS (TOP ROW like your 2nd screenshot)
# ------------------------------
...
# ------------------------------
if predict_btn:
    # Build input exactly with training feature names
    X = pd.DataFrame([{
    ...
    # Try to get probabilities for pie chart (best visual)
    probs = None
    classes = None
    ...
    # ------------------------------
    # CARBON FOOTPRINT (like your 3rd screenshot)
    # Disposed / Recycling / Saved
    # ------------------------------
    ...
    st.caption(
        "*Estimates are simplified emission factors (kg CO₂e per kg waste). "
    ...
    # ------------------------------
    # PIE CHART (must change every time)
    # Use model confidence distribution (predict_proba)
    # ------------------------------
    ...
    if probs is not None and classes is not None:
        ...
        st.warning("This model does not provide prediction probabilities, so pie chart cannot change dynamically.")
